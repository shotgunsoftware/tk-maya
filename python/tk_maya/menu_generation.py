...
class MenuGenerator(object):
    ...
    def create_menu(self, *args):
        ...
        cmds.menu(self._menu_handle, e=True, deleteAllItems=True)
        # now add the context item on top of the main menu
        self._context_menu = self._add_context_menu()
        cmds.menuItem(divider=True, parent=self._menu_handle)
    ...
    def _add_context_menu(self):
        ...
        ctx_menu = cmds.menuItem(label=ctx_name.decode("utf-8"), parent=self._menu_handle, subMenu=True)

        # link to UI
        cmds.menuItem(label="Jump to Shotgun",
                      parent=ctx_menu,
                      command=self._jump_to_sg)

        # Add the menu item only when there are some file system locations.
        if ctx.filesystem_locations:
            cmds.menuItem(label="Jump to File System",
                          parent=ctx_menu,
                          command=self._jump_to_fs)
    ...
    def _add_app_menu(self, commands_by_app):
        ...
        for app_name in sorted(commands_by_app.keys()):
            ...
            if len(commands_by_app[app_name]) > 1:
                ...
                app_menu = cmds.menuItem(label=app_name, parent=self._menu_handle, subMenu=True)
                # get the list of menu cmds for this app
                menu_cmds = commands_by_app[app_name]
                # make sure it is in alphabetical order
                menu_cmds.sort(key=lambda x: x.name) 
...
class AppCommand(object):
    ...
    def _execute_deferred(self, *args):
        """
